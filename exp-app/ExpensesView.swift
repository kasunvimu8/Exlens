...
struct ExpensesView: View {
                    ...
                    AxisMarks(position: .leading, values: .automatic(desiredCount: 3)) { value in
                        AxisValueLabel {
                            if let amount = value.as(Double.self) {
                                Text(amount, format: .currency(code: store.selectedCurrency).precision(.fractionLength(0)))
                                    .font(.system(size: 8))
                            }
                        }
                    }
    ...
    private func categoryColor(for expense: Expense) -> Color {
        if let cat = store.categories.first(where: { $0.id == expense.categoryID }) {
            return Color.fromString(cat.color)
        }
        return .accentColor
    }
    ...
    private func expenseRow(_ expense: Expense) -> some View {
        HStack(spacing: 12) {
            Image(systemName: expense.categoryIcon)
                .font(.body)
                .foregroundStyle(categoryColor(for: expense))
                .frame(width: 36, height: 36)
                .background(categoryColor(for: expense).opacity(0.12), in: RoundedRectangle(cornerRadius: 8))
